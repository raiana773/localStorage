// ! localStorage - это веб-хранилище, то есть данные, хранящиеся докально в браузере пользователя.
// ! localStorage вы можете хранить информацию в фориате ключ-значение.
// ? Ключи и значение всегда строки
// если нам необходимо хранить данные в хранилище как массивы и объекты то нам необходима преоброзовать эти массивы и объекты в строки с помощью метода
// ? JSON.stringify()
...
// ? setItem() принимает ключ и значения.Позволяет добавить новые данные в localStorage - то есть установить для ключа указанного значение, если в хранилище есть данные с таким ключом то они перезаписываются
...
// ! JSON - JavaScript Object Natation и представляет собой текстовый формат для хранения и обмена данными. Говоря простыми словами, это такой формат , который используется для того, чтобы передавать данные между бэкендом и фронтендом и также для хранения данных в localstorage
// ! практика
